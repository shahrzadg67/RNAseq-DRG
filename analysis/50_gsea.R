#!/usr/bin/env Rscript
# ============================================================================
# 50_gsea.R  —  GSEA (KEGG, GO:BP, GO:CC, GO:MF) for every contrast, gene &
# transcript level. Ranking metric = DESeq2 Wald stat (falls back to log2FC).
# Transcript level is collapsed to parent gene (max |stat| per gene) because
# gene-set databases are gene-level.
# Hybrid output: result tables (for interactive plotly/DT) + precomputed
# running-score PNGs (top sets) + pathview KEGG maps.
# ============================================================================
...
GSEA_DIR <- file.path(WWW, "gsea"); dir.create(GSEA_DIR, recursive = TRUE, showWarnings = FALSE)
PV_DIR   <- file.path(WWW, "pathview"); dir.create(PV_DIR, recursive = TRUE, showWarnings = FALSE)
...
for (lv in LEVELS) for (cn in names(CONTRASTS)) {
  ...
  for (cat in CATS) {
    ...
    message("GSEA :: ", key, "  (", length(gl), " genes)")
...
saveRDS(gsea, file.path(APP_DATA, "gsea.rds"))
message("\nGSEA done. result sets: ", length(gsea),
        " | running-score PNGs: ", sum(unlist(plot_counts)),
        " (capped at ", TOP_PLOTS, "/category — extra sets appear in tables without a precomputed plot)")
