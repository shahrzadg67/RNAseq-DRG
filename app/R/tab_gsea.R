# GSEA tab — interactive dotplot + clickable table -> precomputed plots --------
tab_gsea_ui <- function(id) {
  ns <- NS(id)
  nav_panel(
    title = "GSEA (uncorrected)", icon = icon("diagram-project"),
    div(class = "container-fluid py-3",
      h3("Gene Set Enrichment Analysis"),
      div(class = "banner",
          "GSEA over GO (BP/CC/MF) and KEGG for each contrast. Positive NES = enriched among ",
...
tab_gsea_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    output$dot_wrap <- renderUI({
      if (is.null(gsea)) return(coming_soon("GSEA results pending."))
    ...
    output$dot <- plotly::renderPlotly({
      ...
      plotly::plot_ly(d, x = ~NES, y = ~Description,
        ...
        text = ~paste0("<b>", Description, "</b><br>", ID,
                       ...
                       "<br>size: ", setSize),
        hoverinfo = "text") |>
        plotly::layout(title = paste0(input$cat, " — ", input$contrast),
      ...
      DT::datatable(d, rownames = FALSE, selection = "single", filter = "top", extensions = "Buttons",
        ...
        options = list(pageLength = 12, scrollX = TRUE, dom = "Bfrtip",
          buttons = list(list(extend = "excel", text = "⤓ Excel",
                              filename = paste0("GSEA_", input$level, "_", disp(cname()), "_", input$cat))))) |>
    ...
    output$plot_panel <- renderUI({
      ...
      sid <- d$ID[sel]
      png <- file.path("gsea", input$level, cname(), input$cat,
      ...
      if (input$cat == "KEGG") {
        kegg_png <- file.path("pathview", cname(), paste0(sid, ".pathview.png"))
